...
#
# $Id: files.py 221 2011-08-18 14:31:34Z stuwilkins $
...
__date__      = "$LastChangedDate: 2011-08-19 00:31:34 +1000 (Fri, 19 Aug 2011) $"
__id__        = "$Id: files.py 221 2011-08-18 14:31:34Z stuwilkins $"
...
class PrincetonSPEFile():
    ...
    def __str__(self):
        ...
        s += "Timing Mode          : %d\n" % self.TimingMode
        s += "Timing Mode String   : %s\n" % self.timingModeString
        s += "Exposure (s)         : %f\n" % self.exposure
        s += "Delay (s)            : %f\n" % self.delayTrigger
        s += "Det. Temp            : %d\n" % self.DetTemperature
        s += "Det. Type            : %d\n" % self.DetectorType
    ...
    def readData(self):
        ...
        self._readDate()
        self._readArray()
        self._readWavelengthCoeffs()
        self._calcWavelengths()
        self._calcTimings()
    ...
    def _readHeader(self):
        ...
        self.Gain = self._readInt(148)
        self.GeometricOps = self._readInt(600)
        self.WavelengthCoeffs = self._readAtNumpy(3263, 6, numpy.float)
        self.GratingGrooves = self._readFloat(650)
    ...
    # The below functions are all FZ additions to the class

    def _readWavelengthCoeffs(self):
        self.WavelengthCoeffs = self._readAtNumpy(3263, 6, numpy.float)

    def getWavelengthCoeffs(self):
        """Return the coefficents for determining the wavelengths"""
        return self._readAtNumpy(3263, 6, numpy.float)

    def _calcWavelengths(self):
        """Calculate the wavelength range associated with the data"""
        self.Wavelengths = []
        for i in range(self.getSize()[2]):
            self.Wavelengths.append(self.WavelengthCoeffs[0]*i**0 + self.WavelengthCoeffs[1]*i**1 +self.WavelengthCoeffs[2]*i**2 + self.WavelengthCoeffs[3]*i**3 + self.WavelengthCoeffs[4]*i**4 + self.WavelengthCoeffs[5]*i**5)

    def getWavelengths(self):
        return self.Wavelengths

    def _calcTimings(self):
        """Calculate all the timing stuff associated with the *.spe file"""
        # The timing mode is read in furhter up in the code...
        # These are taken from TE's code and his trial and error stuff with the modes
        if (self.TimingMode == 1): # Shutter mode
            self.timingModeString = 'Shutter'
            self.exposure = self._readFloat(10)
            self.delayTrigger = 0.0
        elif (self.TimingMode == 26): # Gate mode
            self.timingModeString = 'Gate Mode'
            self.exposure = self._readFloat(118) / 1.0e6
            self.delayTrigger = self._readFloat(122) / 1.0e6
        else: # Unknown other mode
            self.timingModeString = 'I have no idea what mode I"m in :-o'
            self.exposure = self._readFloat(118) / 1.0e6
            self.delayTrigger = self._readFloat(122) / 1.0e6

    def getTimings(self):
        return self.timingModeString, self.exposure, self.delayTrigger
