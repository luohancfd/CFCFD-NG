...
Energy_exchange_mechanism* create_energy_exhange_mechanism( lua_State * L )
{
    // FIXME - get type from the lua state
    string type = get_string(L, -1, "type");
    
    if( type == "VT_exchange" ) {
	return new VT_exchange(L);
    }
