...
Plow  = 8.0e6                         # Pa
Pinter = 13.3e6                       # Phigh/Pinter = Pinter/Plow
if 0:
    dh_regen = 43.0e3                     # guess for heat regenerated, J/kg
    eta_c = 0.90 # compressor efficiency
    eta_t = 0.90 # turbine efficiency
if 1:
    dh_regen = 43.6e3                    # guess for heat regenerated, J/kg
    eta_c = 0.85 # compressor efficiency
    eta_t = 0.85 # turbine efficiency

fluid = CarbonDioxide()
st = [State() for i in range(26)] # set of CO2 states
...
print "# [4] Exit of brine heatex, inlet to first-stage turbine."
...
# Step up through heat exchanger to get a better poltted curve
nstep = 20
...
print "# [5] Exit of first-stage turbine."
st[5].p = Pinter; st[5].s = st[4].s # specified, isentropic
st[5].rho = st[4].rho/2.0; st[5].T = st[4].T/1.5 # guess
fluid.eval_state(st[5], "ps", 1)
print "# [5]", st[5].str()

print "# [15] imperfect first-stage turbine: efficiency=", eta_t
...
print "# [14] Exit of brine superheat, inlet to second-stage turbine."
st[14].p = st[15].p; st[14].T = Thot # specified
st[14].rho = st[15].rho*st[15].T/st[14].T # guess
fluid.eval_state(st[14], "pt", 1)
print "# [14]", st[14].str()
s_key_points.append(st[14].s/1000)
T_key_points.append(st[14].T)

# Step up through heat exchanger to get a better plotted curve
nstep = 20
dT = (st[14].T - st[15].T)/nstep
for i in range(nstep):
    st[0].p = st[15].p; st[0].T = st[15].T + (i+1)*dT
    st[0].rho = st[15].rho * st[15].T/st[0].T # guess
    fluid.eval_state(st[0], "pt", 1)
    s_cycle.append(st[0].s/1000)
    T_cycle.append(st[0].T)

print "# [25] Exit of second-stage turbine."
st[25].p = Plow; st[25].s = st[14].s # specified, isentropic
st[25].rho = st[14].rho/2.0; st[25].T = st[14].T/1.5 # guess
fluid.eval_state(st[25], "ps", 1)
print "# [25]", st[25].str()

print "# [25] imperfect second-stage turbine: efficiency=", eta_t
st[25].h = st[14].h - eta_t * (st[14].h - st[25].h)
st[25].p = Plow
fluid.eval_state(st[25], "ph", 1)
print "# [25]", st[25].str()
s_cycle.append(st[25].s/1000)
T_cycle.append(st[25].T)
s_key_points.append(st[25].s/1000)
T_key_points.append(st[25].T)

print "# [6] After regenerator."
st[6].p = Plow; st[6].h = st[25].h - dh_regen
st[6].rho = st[25].rho; st[6].T = st[25].T # guess
...
work_from_turbine_1 = st[4].h - st[15].h
work_from_turbine_2 = st[14].h - st[25].h
work_from_turbines = work_from_turbine_1 + work_from_turbine_2
heat_added_1 = st[4].h - st[3].h
heat_added_2 = st[14].h - st[15].h
heat_added = heat_added_1 + heat_added_2
heat_rejected = st[6].h - st[7].h
work_by_pump = st[12].h - st[1].h
cycle_eff = (work_from_turbines - work_by_pump) / heat_added
print "# work-from-turbine_1=", work_from_turbine_1/1000.0, " kJ/kg"
print "# work-from-turbine_2=", work_from_turbine_2/1000.0, " kJ/kg"
print "# work-from-turbines=", work_from_turbines/1000.0, " kJ/kg"
print "# heat-added_1=", heat_added_1/1000.0, " kJ/kg"
print "# heat-added_2=", heat_added_2/1000.0, " kJ/kg"
