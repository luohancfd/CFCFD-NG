...
class BarsFrame(wx.Frame):
	...
	def __init__(self):
		...
		self.textbox.SetValue(str(10000.))
		self.draw_figure()
	...
	def load_models(self):
		...
		for isp in range(nsp):
			species_list.append( self.gm.species_name(isp)
		reactants = make_cea2_dictionary( species_list )
	...
	def calculate_gas_state(self):
		...
		# input data (eventually make user defined)
		massf = [ 0.0 ] * nsp
		massf[self.gm.get_isp_from_species_name("N2")] = 0.767
		massf[self.gm.get_isp_from_species_name("O2")] = 0.233
				
		for iT in range(ntm):
			self.Q.T[iT] = T
		self.Q.p = p
		for isp in range(nsp):
			self.Q.massf[isp] = massf[isp]
		self.gm.eval_thermo_state_pT(self.Q)
	...
	def on_about(self, event):
		 ...
		 * Enter a temperature into the text book and click "Draw!"
