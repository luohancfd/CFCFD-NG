...
# and is based on a set of nenzfr nozzle flow calculations 
# generated by "nenzfr_sensitivity.py"
# 
# Luke Doherty
...
def write_sensitivity_summary(sensitivity, perturbedVariables, exitVar):
    ...
    formatDict = {'p1':'{0:13.4f}', 'T1':'{0:>10.4f}', 'Vs':'{0:>11.4f}',
                      'pe':'{0:>15.4f}', 'Tw':'{0:>10.4f}', 'BLTrans':'{0:>10.4f}',
                      'TurbVisRatio':'{0:>14.4f}', 'TurbInten':'{0:>11.4f}',
...
def write_uncertainty_summary(uncertainty, perturbedVariables, exitVar, inputUncertainties):
    """
    Write out a file summarising the sensitivity of each exit flow parameter to each of
    the input parameters.
    """
    titleFormatDict = {'p1':'{0:{fill}>10}', 'T1':'{0:{fill}>10}', 'Vs':'{0:{fill}>10}',
                       'pe':'{0:{fill}>10}', 'Tw':'{0:{fill}>10}', 'BLTrans':'{0:{fill}>10}',
                       'TurbVisRatio':'{0:{fill}>14}', 'TurbInten':'{0:{fill}>11}',
                       'CoreRadiusFraction':'{0:{fill}>20}'}

    fout = open('uncertainties.dat','w')
    formatDict = {'p1':'{0:10.2%}', 'T1':'{0:>10.2%}', 'Vs':'{0:>10.2%}',
                  'pe':'{0:>10.2%}', 'Tw':'{0:>10.2%}', 'BLTrans':'{0:>10.2%}',
...
def main():
    ...
    op.add_option('--run-defaults', dest='runDefaults', action='store_true',
                  default=True, help="calculate sensitivities and uncertainties using "
                  "all the default values.")
    
    
    op.add_option('--exitStatsfile', dest='exitStatsFileName',
                  default='nozzle-exit.stats',
                  help="file that holds the averaged nozzle-exit data and is to be "
    ...
    op.add_option('--Xpe', dest='pe', default=0.025,
                  help=("equilibrium pressure (after shock reflection), in Pa and its "
                        "perturbation/s as a list. [default: 5%]"))
    ...
    op.add_option('--XBLTrans', dest='BLTrans', default=1.00,
                  help=("relative uncertainty in the boundary layer transition location. "
                        "[default: %default]"))
    op.add_option('--XTurbVisRatio', dest='TurbVisRatio', default=1.00,
                  help=("relative uncertainty in turbulent-to-laminar viscosity ratio "
                        "at the throat. [default: %default]"))
    op.add_option('--XTurbIntensity', dest='TurbInten', default=0.8,
                  help=("relative uncertainty in turbulence intensity at the throat. "
                  "[default: %default]"))
    op.add_option('--XCoreRadiusFraction', dest="coreRfraction", default=0.05,
                  help=("relative uncertainty in the core flow radius fraction. "
                  "[default: %default]")) 
    ...
    # Create a dictionary of the relative uncertainties of each variable that was
    # perturbed
    ...
    for k in range(len(perturbedVariables)):
        ...
        if opt.gradient == "linear":
            ...
            # As the perturbations may not be centered on the nominal condition
            # we caculate the gradient by taking a weighted average of the forward
            # and backward derivatives. The weightings are such that the truncation
            # error associated with this gradient estimate is O(3) or higher (i.e.
            # the weightings are such that the second order terms in the Taylor 
            ...
            # Now calculate the uncertainty in each exit flow variable due to the 
            # uncertainty in the current (perturbed) input variable
    ...
    write_uncertainty_summary(uncertainty, perturbedVariables, exitVar, inputUncertainties)
