...
{
    bd.bcp[NORTH]->apply_viscous_moving_wall(t);
    bd.bcp[EAST]->apply_viscous_moving_wall(t);
    bd.bcp[SOUTH]->apply_viscous_moving_wall(t);
    bd.bcp[WEST]->apply_viscous_moving_wall(t);
    if ( dimensions == 3 ) {
	bd.bcp[TOP]->apply_viscous_moving_wall(t);
	bd.bcp[BOTTOM]->apply_viscous_moving_wall(t);
    }
    return SUCCESS;
}
//-----------------------------------------------------------------------
...
int BoundaryCondition::apply_viscous_moving_wall( double t )
{
    // This will add the tangential velocity due to the movement
    // of solid wall to accout for viscous stress induced by
    // moving grid
    size_t i, j, k;
    FV_Cell *src_cell;
    FV_Interface *IFace;
    ...
    case NORTH:
	    ...
	    j = bd.jmax;
		    ...
		    IFace->fs->vel.y = IFace->ivel.y;
		    IFace->fs->vel.z = IFace->ivel.z;
    ...
    case BOTTOM:
 	...
 	break;
    default:
	printf( "Error: apply_viscous_moving_wall not implemented for boundary %d\n", 
		which_boundary );
	return NOT_IMPLEMENTED_ERROR;
    }    
    return SUCCESS;
} // end BoundaryCondition::apply_viscous_moving_wall()
