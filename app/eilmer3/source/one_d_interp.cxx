...
double overshoot_factor = 1.0; // default keeps the recent clipping behaviour
double set_overshoot_factor(double value)
{
    overshoot_factor = value;
...
inline double clip_to_limits(double q, double A, double B, bool assume_positive_flag)
...
// it returns the closer limit of the range [A,B].
{
	...
	// Let's expand our clipping range to be no so limiting.
	if ( assume_positive_flag ) {
	    upper_limit *= overshoot_factor;
	    lower_limit /= overshoot_factor;
	} else {
	    double mid_range = 0.5*(upper_limit + lower_limit);
	    double half_range = 0.5*(upper_limit - lower_limit);
	    upper_limit = mid_range + (half_range * overshoot_factor);
	    lower_limit = mid_range - (half_range * overshoot_factor);
	}
    }
    return min(upper_limit, max(lower_limit, q));
} // end clip_to_limits()

inline int one_d_interp_both_scalar(double qL1, double qL0, double qR0, double qR1,
				    double &qL, double &qR, bool assume_positive_flag)
{
	...
	qL = clip_to_limits(qL, qL0, qR0, assume_positive_flag);
	qR = clip_to_limits(qR, qL0, qR0, assume_positive_flag);
    }
    return SUCCESS;
...
inline int one_d_interp_left_scalar(double qL1, double qL0, double qR0,
				    double &qL, bool assume_positive_flag)
{
...
inline int one_d_interp_right_scalar(double qL0, double qR0, double qR1,
				     double &qR, bool assume_positive_flag)
{
		      ...
		      FlowState &Lft, FlowState &Rght )
{
	...
	one_d_interp_both_scalar(cL1.fs->vel.x, cL0.fs->vel.x, cR0.fs->vel.x, cR1.fs->vel.x, Lft.vel.x, Rght.vel.x, false);
	one_d_interp_both_scalar(cL1.fs->vel.y, cL0.fs->vel.y, cR0.fs->vel.y, cR1.fs->vel.y, Lft.vel.y, Rght.vel.y, false);
	one_d_interp_both_scalar(cL1.fs->vel.z, cL0.fs->vel.z, cR0.fs->vel.z, cR1.fs->vel.z, Lft.vel.z, Rght.vel.z, false);
	if ( G.MHD ) {
	    one_d_interp_both_scalar(cL1.fs->B.x, cL0.fs->B.x, cR0.fs->B.x, cR1.fs->B.x, Lft.B.x, Rght.B.x, false);
	    one_d_interp_both_scalar(cL1.fs->B.y, cL0.fs->B.y, cR0.fs->B.y, cR1.fs->B.y, Lft.B.y, Rght.B.y, false);
	    one_d_interp_both_scalar(cL1.fs->B.z, cL0.fs->B.z, cR0.fs->B.z, cR1.fs->B.z, Lft.B.z, Rght.B.z, false);
	}
	if ( G.turbulence_model == TM_K_OMEGA ) {
	    one_d_interp_both_scalar(cL1.fs->tke, cL0.fs->tke, cR0.fs->tke, cR1.fs->tke, Lft.tke, Rght.tke, true);
	    one_d_interp_both_scalar(cL1.fs->omega, cL0.fs->omega, cR0.fs->omega, cR1.fs->omega, Lft.omega, Rght.omega, true);
	}
	...
	if ( nsp > 1 ) {
		...
		one_d_interp_both_scalar(gL1.massf[isp], gL0.massf[isp], gR0.massf[isp], gR1.massf[isp],
					 Lft.gas->massf[isp], Rght.gas->massf[isp], true);
	    }
	...
	case INTERP_PT: 
	    one_d_interp_both_scalar(gL1.p, gL0.p, gR0.p, gR1.p, Lft.gas->p, Rght.gas->p, true);
	    for ( size_t i = 0; i < nmodes; ++i ) {
		one_d_interp_both_scalar(gL1.T[i], gL0.T[i], gR0.T[i], gR1.T[i], Lft.gas->T[i], Rght.gas->T[i], true);
	    }
	    ...
	    break;
	case INTERP_RHOE:
	    one_d_interp_both_scalar(gL1.rho, gL0.rho, gR0.rho, gR1.rho, Lft.gas->rho, Rght.gas->rho, true);
	    for ( size_t i = 0; i < nmodes; ++i ) {
		one_d_interp_both_scalar(gL1.e[i], gL0.e[i], gR0.e[i], gR1.e[i], Lft.gas->e[i], Rght.gas->e[i], true);
		// Note that energies are assumed positive for clipping purposes -- Check that this is OK.
	    }
		      ...
		      FlowState &Lft, FlowState &Rght)
{
	...
	one_d_interp_left_scalar(cL1.fs->vel.x, cL0.fs->vel.x, cR0.fs->vel.x, Lft.vel.x, false);
	one_d_interp_left_scalar(cL1.fs->vel.y, cL0.fs->vel.y, cR0.fs->vel.y, Lft.vel.y, false);
	one_d_interp_left_scalar(cL1.fs->vel.z, cL0.fs->vel.z, cR0.fs->vel.z, Lft.vel.z, false);
	if ( G.MHD ) {
	    one_d_interp_left_scalar(cL1.fs->B.x, cL0.fs->B.x, cR0.fs->B.x, Lft.B.x, false);
	    one_d_interp_left_scalar(cL1.fs->B.y, cL0.fs->B.y, cR0.fs->B.y, Lft.B.y, false);
	    one_d_interp_left_scalar(cL1.fs->B.z, cL0.fs->B.z, cR0.fs->B.z, Lft.B.z, false);
	}
	if ( G.turbulence_model == TM_K_OMEGA ) {
	    one_d_interp_left_scalar(cL1.fs->tke, cL0.fs->tke, cR0.fs->tke, Lft.tke, true);
	    one_d_interp_left_scalar(cL1.fs->omega, cL0.fs->omega, cR0.fs->omega, Lft.omega, true);
	}
	Gas_data &gL1 = *(cL1.fs->gas); Gas_data &gL0 = *(cL0.fs->gas); Gas_data &gR0 = *(cR0.fs->gas);
	if ( nsp > 1 ) {
	    // Multiple species.
	    for ( size_t isp = 0; isp < nsp; ++isp ) {
		one_d_interp_left_scalar(gL1.massf[isp], gL0.massf[isp], gR0.massf[isp], Lft.gas->massf[isp], true);
	    }
	    if ( scale_mass_fractions( Lft.gas->massf ) != 0 ) {
		for ( size_t isp=0; isp < nsp; ++isp )
		    Lft.gas->massf[isp] = gL0.massf[isp];
	...
	switch ( thermo_interpolator ) {
	case INTERP_PT: 
	    one_d_interp_left_scalar(gL1.p, gL0.p, gR0.p, Lft.gas->p, true);
	    for ( size_t i = 0; i < nmodes; ++i ) {
		one_d_interp_left_scalar(gL1.T[i], gL0.T[i], gR0.T[i], Lft.gas->T[i], true);
	    }
	    if ( gmodel->eval_thermo_state_pT(*(Lft.gas)) != SUCCESS ) {
		Lft.copy_values_from(*(cL0.fs));
	    }
	    break;
	case INTERP_RHOE:
	    one_d_interp_left_scalar(gL1.rho, gL0.rho, gR0.rho, Lft.gas->rho, true);
	    for ( size_t i = 0; i < nmodes; ++i ) {
		one_d_interp_left_scalar(gL1.e[i], gL0.e[i], gR0.e[i], Lft.gas->e[i], true);
	    }
	    if ( gmodel->eval_thermo_state_rhoe(*(Lft.gas)) != SUCCESS ) {
		Lft.copy_values_from(*(cL0.fs));
	    }
		       ...
		       FlowState &Lft, FlowState &Rght)
{
	...
	one_d_interp_right_scalar(cL0.fs->vel.x, cR0.fs->vel.x, cR1.fs->vel.x, Rght.vel.x, false);
	one_d_interp_right_scalar(cL0.fs->vel.y, cR0.fs->vel.y, cR1.fs->vel.y, Rght.vel.y, false);
	one_d_interp_right_scalar(cL0.fs->vel.z, cR0.fs->vel.z, cR1.fs->vel.z, Rght.vel.z, false);
	if ( G.MHD ) {
	    one_d_interp_right_scalar(cL0.fs->B.x, cR0.fs->B.x, cR1.fs->B.x, Rght.B.x, false);
	    one_d_interp_right_scalar(cL0.fs->B.y, cR0.fs->B.y, cR1.fs->B.y, Rght.B.y, false);
	    one_d_interp_right_scalar(cL0.fs->B.z, cR0.fs->B.z, cR1.fs->B.z, Rght.B.z, false);
	}
	if ( G.turbulence_model == TM_K_OMEGA ) {
	    one_d_interp_right_scalar(cL0.fs->tke, cR0.fs->tke, cR1.fs->tke, Rght.tke, true);
	    one_d_interp_right_scalar(cL0.fs->omega, cR0.fs->omega, cR1.fs->omega, Rght.omega, true);
	}
	Gas_data &gL0 = *(cL0.fs->gas); Gas_data &gR0 = *(cR0.fs->gas); Gas_data &gR1 = *(cR1.fs->gas);
	if ( nsp > 1 ) {
	    // Multiple species.
	    for ( size_t isp = 0; isp < nsp; ++isp ) {
		one_d_interp_right_scalar(gL0.massf[isp], gR0.massf[isp], gR1.massf[isp], Rght.gas->massf[isp], true);
	    }
	    if ( scale_mass_fractions( Rght.gas->massf ) != 0 ) {
		for ( size_t isp=0; isp < nsp; ++isp )
		    Rght.gas->massf[isp] = gR0.massf[isp];
	...
	switch ( thermo_interpolator ) {
	case INTERP_PT: 
	    one_d_interp_right_scalar(gL0.p, gR0.p, gR1.p, Rght.gas->p, true);
	    for ( size_t i = 0; i < nmodes; ++i ) {
		one_d_interp_right_scalar(gL0.T[i], gR0.T[i], gR1.T[i], Rght.gas->T[i], true);
	    }
	    if ( gmodel->eval_thermo_state_pT(*(Rght.gas)) != SUCCESS ) {
		Rght.copy_values_from(*(cR0.fs));
	    }
	    break;
	case INTERP_RHOE:
	    one_d_interp_right_scalar(gL0.rho, gR0.rho, gR1.rho, Rght.gas->rho, true);
	    for ( size_t i = 0; i < nmodes; ++i ) {
		one_d_interp_right_scalar(gL0.e[i], gR0.e[i], gR1.e[i], Rght.gas->e[i], true);
	    }
	    if ( gmodel->eval_thermo_state_rhoe(*(Rght.gas)) != SUCCESS ) {
		Rght.copy_values_from(*(cR0.fs));
	    }
