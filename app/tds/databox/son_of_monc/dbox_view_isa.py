...
from dbox_services_isa import *
from dbox_data_isa import *
...
print "Initialize..."
## The following is troublesome... but it would be nice to have working.
## if not os.path.exists("/dev/databox/trigger_control"):
##     # Start up the device driver since it appears not to be running.
##     print "Start our device driver"
##     os.system("../device_driver/dbox_driver 2> /dev/null &")
##     time.sleep(2)
## else:
##     print "Device driver already running."
...
def do_collect_data():
    ...
    for card_id in cardList:
        ...
        text_data = dbox_service.read_card_status(card_id)
        card_status = dbox_service.decode_card_status(text_data, 1)
    ...
    if ncss > 0:
        ...
        # Do the collection.
        busy()
        text_data = dbox_service.read_timebase_status()
        timebase_status = dbox_service.decode_timebase_status(text_data, 1)
        ...
        for card_id in cardList:
            ...
            for channel_id in card.channels_used.keys():
                ...
                binary_data = dbox_service.read_data_buffer(card_id, channel_id)
                voltage_demux, ground_input, FS = \
                    dbox_service.decode_data_buffer(binary_data, card_status["first_word"],
                                                    buffer_size, mux, 1)
                if mux < 1:
                    # No multiplexing
                    subchannel_id = 0
        ...
        pd.close()
        not_busy()
        dialog = Pmw.MessageDialog(root, title="Collect data finished",
                     defaultbutton=0, buttons=("OK",),
                     message_text="Data collected from cards %s." % str(cardList))
...
def save_data_dialog():
    ...
    if result == "Proceed":
        ...
        dialog = Pmw.MessageDialog(root, title="Saving data.",
                     defaultbutton=0, buttons=("OK",),
                     message_text="Finished saving data.")
        result = dialog.show()
        root.update()
        wait_a_while(200) # seem to need a bit of time to clear dialog
    return
...
def refresh_databox_status_page(rescheduleCheckWhenDone=0):
    ...
    if dbox_info.lock_is_free():
        ...
        #
        for i in range(1,4):
            text_data = dbox_service.read_trigger_status(i)
            trs = dbox_service.decode_trigger_status(text_data)
            display_data = "slope=%s  coupling=%s  threshold=%6.0f" % \
                           (trs["slope"][i], trs["coupling"][i], trs["threshold"])
            triggerStatusEntryWidget[i].insert(Tkinter.END, display_data)
        #
        text_data = dbox_service.read_timebase_status()
        tbs = dbox_service.decode_timebase_status(text_data)
        for i in range(1,4):
        ...
        for card_id in dbox_info.cardDict.keys():
            card = dbox_info.cardDict[card_id]
            text_data = dbox_service.read_card_status(card_id)
            cs = dbox_service.decode_card_status(text_data)
...
def layout_plot_page(parent):
    ...
    plotsPerRowComboBox.selectitem("1")
    ...
    c = Tkinter.Canvas(scf, bg="gray85", width=canvasWidth, height=400,
...
def plot_a_signal(signal, xleft, ytop, xsize, ysize):
    ...
    if N > 0:
        ...
        # The following code uses lists only.
        # coordList = []
        # for i in range(0, N, nth):
        #     x = x1 + int(xaxis_length * float(i) / N)
        #     y = y0 - int(signal.raw_data[i] / signal.FS * yaxis_length)
        #     coordList.append(x)
        #     coordList.append(y)
