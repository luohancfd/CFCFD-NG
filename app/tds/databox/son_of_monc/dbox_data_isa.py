...
""" % mx.DateTime.now()
        ...
        self.lock = 0
        try:
            if dbox_service.device_driver_is_running() and \
                   dbox_service.databox_is_present():
                text_data = dbox_service.read_timebase_status()
                timebase_info = dbox_service.decode_timebase_status(text_data, 1)
...
def write_scaled_data_to_file(dbox_info, signal, f, extn):
    ...
    f.write("# dataSource dbox_view v1.0 2004\n")
    f.write("# dateTime %s\n" % mx.DateTime.now())
    f.write("# shotName %s\n" % dbox_info.shotId)
    f.write("# channelId %s\n" % extn)
