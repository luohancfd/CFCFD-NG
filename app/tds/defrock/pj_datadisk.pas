...
begin
   {$I-}
   name:=filename[2]+'.MOD';
   writeln('Look for model file: ',name);
   assign(fname,name);
   reset(fname);
   if ioresult<>0 then
   begin
      write('Did not find file: '+name+'; ');
      name := filename[2]+'.mod';
      writeln('trying '+name+' instead.');
      assign(fname,name);
      reset(fname)
   end;
   if ioresult = 0 then
   begin
	 ...
	 if ioresult<>0 then serial_number[i]:='';
      ...
      writeln('Done reading model file.');
   end
   else
   begin
      writeln('Cannot find model file.');
   end
   {$I+}
end;
...
begin
     {$I-}
     name:=filename[1]+'.HED';
     writeln('Look for header file ', name);
     assign(fname,name);
     reset(fname);
     if ioresult <> 0 then
     begin
	write('Did not find '+name+'; ');
	name := filename[1] + '.hed';
	writeln('trying '+name+' instead.');
	assign(fname,name);
	reset(fname);
     ...
     if ioresult = 0 then
     begin
	writeln('Reading header file: ', name);
	read(fname,realvar);
	...
	if ioresult<>0 then writeln('Error reading hed file');
     ...
     begin
	writeln('Could not find header file.');
     end;
end;
