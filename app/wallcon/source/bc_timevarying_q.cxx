...
BC_TIMEVARYING_Q::BC_TIMEVARYING_Q(){}

BC_TIMEVARYING_Q::BC_TIMEVARYING_Q(std::vector<double> qwall)
    : qwall(qwall)
{}

void BC_TIMEVARYING_Q::apply_bc(SolidBlock *blk, int which_boundary )
{
    ...
    case 1:
	{
		...
		blk->block_cells[i][j].iface[1]->T = -1*(-qwall[j]/blk->block_cells[i][j].k * norm2_vector( length,0,length.size() ) + blk->block_cells[i][j].T );
	    }
	    break;
	}
    case 2:
	{
		...
		blk->block_cells[i][j].iface[2]->T = -1*(-qwall[i]/blk->block_cells[i][j].k * norm2_vector( length,0,length.size() ) + blk->block_cells[i][j].T );
	    }
	    break;
	}
    case 3:
...
void BC_TIMEVARYING_Q::print_type(){
