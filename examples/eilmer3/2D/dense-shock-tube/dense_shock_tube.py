#!/usr/bin/env python
"""
dense_shock_tube.py -- A dense real-gas shock tube example, done as an example
of using gas_flow functions but can be compared the Eilmer3 shock tube example.

Adapted from the classic-shock-tube gasdyn example.
...
def main():
    print "CO2 driver gas"
    state4 = Gas()
    state4.set_pT(6.0e6, 1000.0)
    print "state4:"
    state4.write_state(sys.stdout)
    #
    print "CO2 driven gas"
    state1 = Gas()
