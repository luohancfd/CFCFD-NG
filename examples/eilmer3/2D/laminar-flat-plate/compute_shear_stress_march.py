...
print "Pick up data..."
grid, flow, dimensions = read_all_blocks(rootName="lam_flat_plate_march", 
                                         nblock=88, tindx=1, zipFiles=1)

print "Compute viscous parameters for cell-centres along plate surface:"
outfile = open("cf-eilmer3-march.data", "w")
outfile.write("# 1.x(m) 2.du(m/s) 3.dy(m) 4.tau_w(N/m^2) 5.c_f 6.y_plus \n")

wall_block_index_list = range(3,22*4,4) # along the NORTH wall.
print "Extract data from blocks:", wall_block_index_list
