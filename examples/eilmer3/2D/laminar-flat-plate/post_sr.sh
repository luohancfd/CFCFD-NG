...
e3post.py --job=$JOB --output-file=profile-at-x1m-sr.dat --tindx=$TINDX \
    --slice-at-point="12,jk,1.0,0.1,0.0;13,jk,1.0,0.1,0.0;14,jk,1.0,0.1,0.0;15,jk,1.0,0.1,0.0" \
    --add-pitot-p --add-mach

awk -f integral-thicknesses.awk profile-at-x1m-sr.dat > thicknesses-sr.txt

gnuplot<<EOF
set term postscript eps enhanced 20
set output "u-velocity-comparison-sr.eps"
...
set xrange [0:1.2]
set yrange [0:20]
plot "./profile-at-x1m-sr.dat" using ((\$6)/1390.0):((0.44-\$2)*1000) \
     title "Eilmer3-sr" with points pt 1, \
...
gnuplot<<EOF
set term postscript eps enhanced 20
set output "density-comparison-sr.eps"
...
set xrange [0:1.2]
set yrange [0:20]
plot "./profile-at-x1m-sr.dat" using ((\$5)/0.011835):((0.44-\$2)*1000) \
     title "Eilmer3-sr" with points pt 1, \
...
# and y+ values.
e3post.py --job=$JOB --output-file=first-cell-off-wall-sr.dat --tindx=$TINDX \
    --slice-list="3,:,-1,0;7,:,-1,0;11,:,-1,0;13,:,-1,0" \
    --add-pitot-p --add-mach

python compute_shear_stress_sr.py
python reference_temperature_method.py

gnuplot<<EOF
set term postscript eps enhanced 20
set output "cf-comparison-sr.eps"
...
set yrange [0:0.004]
set key top right
plot "cf-eilmer3-sr.data" using (\$1):(\$5) title "Eilmer3-sr" with points pt 1, \
