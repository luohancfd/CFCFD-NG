...
omega = 2000				   -- rotor rotational speed [rad/s]
...
function get_flow_downstream(alpha, beta, C, x, y)
   -- Function that calculates absolute cartesian velocities for the moving rotor.
      ...
      for j = 2, nnj+1 do 
   	 ...
   	 ratio = 1+0.5*(g-1)*M_sq				-- T0/T [-]
         ...
         p0 = cell_flow.p*math.pow(ratio,(g/(g-1)))    		-- Total pressure [Pa]
          ...
          --u_theta, u_r = get_cylindrical_flow(cell_flow.u, cell_flow.v, cell_flow.x, cell_flow.y)
         alpha = math.atan(u_theta_abs/cell_flow.w)             -- Inflow angle beta
...
function ghost_cell(args)
   ...
   -- Calculate flow states for ghost cells 
   ...
   ghost.u, ghost.v, ghost.w = get_flow_downstream(alpha[k], beta[k], vel, cell_flow.x, cell_flow.y)
