...
function convective_flux(args)
   ...
   --print("\n")
   --print("Stator outlet, working on: Block,I,J,K",block_id,args.i,args.j,args.k)
   vtx3 = sample_vtx(block_id,args.i,args.j+1,args.k)
   vtx0 = sample_vtx(block_id,args.i,args.j,args.k)
   -- vtx4 = sample_vtx(block_id,args.i,args.j,args.k+1)
   -- vtx7 = sample_vtx(block_id,args.i,args.j+1,args.k+1)
   ...
   for i=1,Z_Nfaces do
      for j=1,T_count do
         ...
         if args.t_step == 0 then   
            flow = sample_flow(b_id,T_faces[j],2,Z_face_ind[i])
            flow2 = sample_j_face(b_id,T_faces[j],2,Z_face_ind[i])
            area = flow2.area
         else 
            flow = sample_j_face(b_id,T_faces[j],2,Z_face_ind[i])
            area = flow.area
         end
   ...
   --   print(F.mass, F.momentum_x,F.momentum_y,F.momentum_z)
   --   print(u,v,w)
   --end
