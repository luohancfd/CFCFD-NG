...
omega = -4031.71057211 				   -- rotor rotational speed [rad/s]
...
function get_relative_flow(alpha, beta, C, x, y)
   ...
   u_theta = C*math.sin(alpha)*math.cos(beta) 
   w = C*math.sin(beta)

   u_r = -C*math.cos(alpha)*math.cos(beta)
...
function get_flow_upstream()
   ...
   --  Calculate relative total pressure and total temperature
   i = nni+1
   T0_radial_rho_av = {}
   p0_radial_rho_av = {}
   alpha_radial_rho_av = {}
   ...
   for k = 2, nnk+1 do
      ...
      rho_sum = 0
      for j = 2, nnj+1 do 
         cell_flow = sample_flow(block_id-1, i, j, k)        	-- Get flow in upstream block 

         u_theta, u_r, radius = get_cylindrical_flow(cell_flow.u, cell_flow.v, cell_flow.x, cell_flow.y)  --u_theta_rel --> u_theta
--       convert to relative frame of reference	
         u_theta = u_theta - omega * radius
	
         vel_sq = u_theta^2 + u_r^2 + cell_flow.w^2    	-- Square of inflow velocity [m^2/s^2]

   	 vel = math.sqrt(vel_sq)                   		-- Inflow velocity [m/s]
--         print ("radius = ", r, "vel =", vel, "u_th =", u_theta)  -- debug info
   	 ...
   	 ratio = 1+0.5*(g-1)*M_sq				-- T0/T [-]
         ...
         alpha = math.atan(u_theta/-u_r)             	-- Inflow angle beta
         beta = math.asin(cell_flow.w/vel)	                        -- Inflow angle alpha
...
function ghost_cell(args)
   ...
   if count > nnj*nnk*2*TimeStep then
      ...
      --alpha, beta, T0, p0 = 0.0, 0.0, 300.0, 100.0e3
   end
...
-- use relative velocity
   vel_sq = u_theta_rel^2+u_r^2+cell_flow.w^2   		-- Square of inflow velocity [m^2/s^2]
   vel = math.sqrt(vel_sq)                   			-- Inflow velocity [m/s]
   h0 = Cp*T0[k]							-- Enthalpy
...
--   print ("k = ", k, "pres =", ghost.p)  -- debug info
   ghost.T = {}
