...
gdata.title = "Method of Manufactured Solutions 3D, Case=%d." % case
gdata.dimensions = 3
...
initial = FlowCondition(p=p0, u=0.0, v=u0, w=v0, T=T0, massf=[1.0,])

a = Node(0.0, 0.0, 0.0, label="a")
b = Node(0.0, 1.0, 0.0, label="b")
c = Node(0.0, 0.0, 1.0, label="c")
d = Node(0.0, 1.0, 1.0, label="d")
x0 = Vector(0.0,0.0,0.0)
x1 = Vector(0.1,0.0,0.0)
pface = CoonsPatch(Line(a,b), Line(c,d), Line(a,c), Line(b,d))
pvol = WireFrameVolume(pface,Line(x0,x1))
...
if case == 1 or case == 3:
    ...
    bc_list = [UserDefinedBC("udf-bc.lua"),]*6

if blocking == 'single':
    blk = Block3D(pvol, nni=nn, nnj=nn, nnk=4, bc_list=bc_list,
                  fill_condition=initial, label="blk")
elif blocking == 'multi':
    blk = SuperBlock3D(pvol, nni=nn, nnj=nn, nnk=4, nbi=4, nbj=4, nbk=1,
                       bc_list=bc_list, fill_condition=initial, label="blk")
else:
